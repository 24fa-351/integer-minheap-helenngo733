...
unsigned int heap_level(unsigned int index) {
    ...
    while (index > 0) {
        index = heap_parent(index);  // move up to the parent
        level++;
    }
    return level;
}
...
void heap_bubble_up(heap_t *heap, int index) {
    while (index > 0 && heap->data[index].key < heap->data[heap_parent(index)].key) {
        // swap with parent if current key is smaller
...
void heap_bubble_down(heap_t *heap, int index) {
    ...
    // check if left and right child exist and if they're smaller than the minimum
